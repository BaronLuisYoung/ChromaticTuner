...
// Set background RGB color for next drawing
void setColorVB(u8 r, u8 g, u8 b) {
	// 5-bit r, 6-bit g, 5-bit b
	vch = (r & 0x0F8) | g >> 5;
	vcl = (g & 0x1C) << 3 | b >> 3;
}
...
int normalPattern(int x, int y) {
	...
	return ((0 <= xMod) && (xMod < 5)) || ((35 <= xMod) && (xMod < 40))
			|| ((0 <= yMod) && (yMod < 5)) || ((35 <= yMod) && (yMod < 40));
}
...
void printChar(u8 c, int x, int y) {
	...
	for (j = 0; j < (cfont.x_size >> 3) * cfont.y_size; j++) {
		...
		for (i = 0; i < 8; i++) {
			if ((ch & (1 << (7 - i))) != 0)
				LCD_Write_DATA16(black_ch, black_cl);
			else
				LCD_Write_DATA16(white_ch, white_cl);
		}
		pixelIndex++;
	}
...
int triangle(int x, int y) {
	// returns 1 if the pixel is a part of the triangle
	//returns 0 if the pixel is a part of the background
	x = x % 40; // map x into 40 pixels of bounding box
	y = y % 40;
	x = abs(20 - x);
	return (y >= 2 * x);
}

void printShape(int x, int y, int w, int h, int (*func)(int i, int j)) {
	if(w==0||h==0)return;
	int i, j, val;
